function [alldoflag]=ReconPHiLo(waveL,theta,raw_file,wf_file,PWF_file, hilo_file, philo_file, philo_ouf_file, philo_ouf_pseudo_file, philo_ouf_pseudo_bar_file, cm_file,stacknum_file,isstackwrite,If_correction,dir_corr)
...
%         HiLo = HiLo/max(HiLo(:));
    ...
    HiLo = (HiLo_sum);
    HiLo_dir=HiLo_dir./max(HiLo_dir(:));
    ...
    [ouf_pHiLo, pHiLo, ~, cm, ld_ouf, h1] = recon_pm(ld, theta, min(ld(:)), max(ld(:)), calib,ratio1,ratio2,hue1,hue2);
    ...
    [~, pWF, ~, ~, ~, ~] = recon_pm(ld, theta,min(ld(:)), max(ld(:)), calib,ratio1,ratio2,hue1,hue2);
    %% saving
    WF1=WF;
